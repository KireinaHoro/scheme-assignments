...
(define (ripple-carry-adder wire-lstA wire-lstB wire-lstS wireCN)
  ;;
  (define c-out (mappend (make-wire-list-N (sub1 (mlength wire-lstA)))
                        (list wireCN)))
  (define c-in (cons (make-wire) c-out))
  (for ([a (in-mlist wire-lstA)]
        [b (in-mlist wire-lstB)]
        [s (in-mlist wire-lstS)]
        [ci (in-mlist c-in)]
        [co (in-mlist c-out)])
