...
(define (list->cons exp)
  (if (null? exp)
      '()
      (list 'cons (list 'quote (car exp)) (list->cons (cdr exp)))))
...
;; syntax rules
(define eval-rules
  (list (cons 'quote (lambda (exp env)
                       
                       (if (pair? (cadr exp))
                           (my-eval (list->cons (cadr exp)) env)
...
(define (setup-environment)
  (let ([initial-env
    ...
    (define-variable! 'cons (my-eval '(lambda (x y) (lambda (m) (m x y))) initial-env) initial-env)
    (define-variable! 'car (my-eval '(lambda (z) (z (lambda (p q) p))) initial-env) initial-env)
    (define-variable! 'cdr (my-eval '(lambda (z) (z (lambda (p q) q))) initial-env) initial-env)
    initial-env))
